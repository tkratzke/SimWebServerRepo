...
public class ModelWriter {
	...
	public Element createModelElement(final SimCaseManager.SimCase simCase,
			final Model model) {
		final Element mainElement = _formatter.newElement("SIM");
		mainElement.setAttribute(ModelReader._WriteOcTablesAtt,
				"" + model.getWriteOcTables());
		final String xmlSimPropertiesFilePath =
				model.getXmlSimPropertiesFilePath();
		if (xmlSimPropertiesFilePath != null) {
			mainElement.setAttribute(ModelReader._XmlSimPropertiesFilePathAtt,
					xmlSimPropertiesFilePath);
		}

		/** Start with the environment. */
		final Element envDataElement =
				_formatter.newChild(mainElement, "ENVDATA");
		/** Winds. */
		final Element outputWindsElement =
				_formatter.newChild(envDataElement, "WIND");
		final Element inputWindsElement = model.getWindsElement();
		final String directionFrom = inputWindsElement == null ? null :
				inputWindsElement.getAttribute("directionFrom");
		...
		if (inputWindsElement != null) {
			final String confidenceAttribute =
					inputWindsElement.getAttribute(ModelReader._ConfidenceAtt);
			final boolean haveConfidenceAttribute =
					confidenceAttribute != null && confidenceAttribute.length() > 0;
			if (!haveConfidenceAttribute) {
				outputWindsElement.setAttribute(ModelReader._ConfidenceAtt,
						"Not_Supplied");
			} else {
				outputWindsElement.setAttribute(ModelReader._ConfidenceAtt,
						confidenceAttribute);
				...
				final boolean havePreConfidenceAttribute =
						confidenceAttribute != null && confidenceAttribute.length() > 0;
				if (havePreConfidenceAttribute) {
					outputWindsElement.setAttribute(
							ModelReader._PreDistressConfidenceAtt,
		...
		if (windsUvGetter == null) {
			...
			model.setEnvironment(simCase, /* stashEnvFiles= */false,
					/* overwriteEnvFiles = */false);
			windsUvGetter = model.getWindsUvGetter();
		}
		windsUvGetter.writeElement(outputWindsElement, inputWindsElement,
				model);

		/** Currents. */
		final Element outputCurrentsElement =
				_formatter.newChild(envDataElement, "CURRENT");
		final Element inputCurrentsElement = model.getCurrentsElement();
		if (inputCurrentsElement != null) {
			String confidenceAttribute =
					inputCurrentsElement.getAttribute(ModelReader._ConfidenceAtt);
			confidenceAttribute =
					confidenceAttribute == null ? "" : confidenceAttribute.trim();
			final boolean haveConfidenceAttribute =
					confidenceAttribute.length() > 0;
			if (haveConfidenceAttribute) {
				outputCurrentsElement.setAttribute(ModelReader._ConfidenceAtt,
						confidenceAttribute);
				String preConfidenceAttribute =
						inputCurrentsElement.getAttribute(ModelReader._ConfidenceAtt);
				preConfidenceAttribute = preConfidenceAttribute == null ? "" :
						preConfidenceAttribute.trim();
				final boolean havePreConfidenceAttribute =
						preConfidenceAttribute.length() > 0;
				if (havePreConfidenceAttribute) {
					outputCurrentsElement.setAttribute(
							ModelReader._PreDistressConfidenceAtt,
							preConfidenceAttribute);
				}
			}
		}
		...
		currentsUvGetter.writeElement(outputCurrentsElement,
				inputCurrentsElement, model);
		...
		for (int scenarioIndex = 0; scenarioIndex < nScenarii;
				++scenarioIndex) {
			model.getScenario(scenarioIndex).write(_formatter, mainElement,
					model);
		}
		...
		addGraphicsElement(_formatter, mainElement,
				model.getExtraGraphicsObject());

		/** Fixed hazards. */
		final Iterator<FixHazard> fixHazardIterator =
				model.getFixHazards().iterator();
		...
		final Element requestElement =
				_formatter.newChild(mainElement, "REQUEST");
		final String typeString =
				model.getReverseDrift() ? "RunReverseDrift" : "RunSimulation";
		requestElement.setAttribute("type", typeString);
		final Element inputElement =
				_formatter.newChild(requestElement, "INPUT");
		inputElement.setAttribute("datumTime",
				TimeUtilities.formatTime(model.getLastOutputRefSecs(), false));
		...
		if (modeName != null) {
			...
			inputElement.setAttribute("particles",
					LsFormatter.StandardFormat(nParticlesPerScenario));
			inputElement.setAttribute("timeStep",
					LsFormatter.StandardFormat(monteCarloTimeStepSecs / 60) +
							" mins");
		}
		final boolean riverine = model.isRiverine();
		inputElement.setAttribute("river", riverine ? "yes" : "no");
		/**
		 * We're just writing out the interpolation mode so we pretend we're
		 * getting it for currents; that's just a "parroting."
		 */
		...
		inputElement.setAttribute("proportionOfSticky",
				String.format("%.2f", proportionOfSticky));
		inputElement.setAttribute("randomSeed",
				LsFormatter.StandardFormat(model.getRandomSeed()));
		...
		inputElement.setAttribute("excludeInitialLandDraws",
				(model.getExcludeInitialLandDraws()) ? "true" : "false");
		inputElement.setAttribute("excludeInitialWaterDraws",
				(model.getExcludeInitialWaterDraws()) ? "true" : "false");
		final Element outputElement =
				_formatter.newChild(requestElement, "OUTPUT");
		outputElement.setAttribute("firstOutputTime",
				TimeUtilities.formatTime(model.getFirstOutputRefSecs(), false));
		outputElement.setAttribute("file", model.getParticlesFilePath());
		return mainElement;
	}

	public static void writeConstantDistribution(final Element outputElement,
			final Element inputElement) {
		outputElement.setAttribute("type", "constant");
		if (inputElement != null) {
			outputElement.setAttribute("speed",
					inputElement.getAttribute("speed"));
	...
	public static void attachWaypoints(final LsFormatter formatter,
			final Element patternElement, final Sortie sortie) {
		...
		for (int legIndex = 0; legIndex < numberOfLegs; ++legIndex) {
			...
			Element wayPointElement =
					formatter.newChild(patternElement, "WAYPOINT");
			wayPointElement.setAttribute("lat", LsFormatter
					.StandardFormatForLatOrLng(leg.getLegLatLng0().getLat()));
			wayPointElement.setAttribute("lng", LsFormatter
					.StandardFormatForLatOrLng(leg.getLegLatLng0().getLng()));
			wayPointElement.setAttribute("dtg",
					TimeUtilities.formatTime(leg.getLegRefSecs0(), false));
			wayPointElement.setAttribute("dtgs",
					TimeUtilities.formatTime(leg.getLegRefSecs0(), true));
			if (legIndex != numberOfLegs - 1) {
				...
				if (!nextLeg.getLegLatLng0().equals(leg.getLegLatLng1())) {
					...
					wayPointElement.setAttribute("lat", LsFormatter
							.StandardFormatForLatOrLng(leg.getLegLatLng1().getLat()));
					wayPointElement.setAttribute("lng", LsFormatter
							.StandardFormatForLatOrLng(leg.getLegLatLng1().getLng()));
					wayPointElement.setAttribute("dtg",
							TimeUtilities.formatTime(leg.getLegRefSecs1(), false));
					wayPointElement.setAttribute("dtgs",
							TimeUtilities.formatTime(leg.getLegRefSecs1(), true));
				}
			} else {
				wayPointElement = formatter.newChild(patternElement, "WAYPOINT");
				wayPointElement.setAttribute("lat", LsFormatter
	...
	public static void attachCompObjectTypes(final LsFormatter formatter,
			final Element element, final Sortie sortie) {
		for (final Map.Entry<Integer, LrcSet> entry : sortie.getViz1LrcSets()
				.entrySet()) {
			final LrcSet lrcSet = entry.getValue();
			final int searchObjectTypeId = entry.getKey();
			final Element soElement =
					formatter.newChild(element, "COMP_OBJECT_TYPE");
			soElement.setAttribute("id",
					LsFormatter.StandardFormat(searchObjectTypeId));
			lrcSet.write(formatter, soElement);
		}
	}

	private static Element addCompletedSearchElement(
			final LsFormatter formatter, final Element root,
			final Sortie sortie) {
		final Element element =
				(root == null) ? formatter.newElement("COMPLETED_SEARCH") :
			...
			if (0d <= creepHdgIn && creepHdgIn < 360d) {
				patternElement.setAttribute("creepDirection",
						String.format("%s degrees clockwise from north",
								LsFormatter.StandardFormat(creepHdgIn)));
			}
		...
		final LatLng3 nwLatLng =
				LatLng3.getLatLngB(extent.getLeftLng(), extent.getMaxLat());
		...
		final LatLng3 seLatLng =
				LatLng3.getLatLngB(extent.getRightLng(), extent.getMinLat());
		...
		element.setAttribute("start",
				TimeUtilities.formatTime(startRefSecs, false));
		final long stopTimeRefSecs = sortie.getStopRefSecs();
		element.setAttribute("end",
				TimeUtilities.formatTime(stopTimeRefSecs, false));
		return element;
	}

	public static void addGraphicsElement(final LsFormatter formatter,
			final Element root, final ExtraGraphicsClass extraGraphics) {
		...
		for (final Graphics graphicsEntry : graphics) {
			...
			final Element graphicsElement =
					formatter.newChild(element, type.name());
			...
			switch (type) {
			case CIRCLE:
				graphicsElement.setAttribute("CenterLat",
						String.format("%.3f", parameters[0]));
				graphicsElement.setAttribute("CenterLng",
						String.format("%.3f", parameters[1]));
				graphicsElement.setAttribute("Radius",
						String.format("%.3f NM", parameters[2]));
				break;
			case ELLIPSE:
				graphicsElement.setAttribute("CenterLat",
						String.format("%.3f", parameters[0]));
				graphicsElement.setAttribute("CenterLng",
						String.format("%.3f", parameters[1]));
				graphicsElement.setAttribute("SemiMajorInNmi",
						String.format("%.3f NM", parameters[2]));
				graphicsElement.setAttribute("SemiMinorInNmi",
						String.format("%.3f NM", parameters[3]));
				graphicsElement.setAttribute("Orientation",
						String.format("%.3f degs", parameters[4]));
				break;
			case LL_GRID_GAPS:
				graphicsElement.setAttribute("LatGap",
						String.format("%.7f", parameters[0]));
				graphicsElement.setAttribute("LngGap",
						String.format("%.7f", parameters[1]));
				break;
			case LL_EDGE:
				...
				if (refSecs0 >= 0) {
					final String t0String =
							TimeUtilities.formatTime(refSecs0, includeSecs);
					graphicsElement.setAttribute("time0", t0String);
				}
				final long refSecs1 = Math.round(parameters[6]);
				if (refSecs1 >= 0) {
					final String t1String =
							TimeUtilities.formatTime(refSecs1, includeSecs);
	...
	public static void addDoubleAttribute(
			final SimCaseManager.SimCase simCase, final Element e,
			final String attributeName, final double d) {
		if (Double.isNaN(d)) {
			SimCaseManager.err(simCase,
					String.format(
							"Converting an NaN to 0 for Element[%s], Attribute[%s]",
