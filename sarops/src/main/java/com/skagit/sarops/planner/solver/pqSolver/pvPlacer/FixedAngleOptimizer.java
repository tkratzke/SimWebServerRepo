...
public class FixedAngleOptimizer {
	...
	private static class IndexFinder {
		...
		private IndexFinder(final double mid, final double cellWidth,
				final int nCells) {
			_mid = mid;
		...
		public String getString() {
			...
			final String s = String.format(
					"MidNmi[%.2f] CellWidthNmi[%.2f] (lowIdx,HghIdx)=(%d,%d)", midNmi,
	...
	public class CornersAndWeights implements Comparable<CornersAndWeights> {
		...
		public CornersAndWeights(final LatLng3[] corners,
				final HashMap<IntDouble, IntDouble> objTpToMass) {
			_corners = corners;
	...
	public FixedAngleOptimizer(final PatternVariable pv,
			...
			final HashMap<IntDouble, IntDouble> objTpToSweepWidth,
			final double firstLegHdg, final double avgSw) {
		...
		final Iterator<Map.Entry<LatLng3, double[]>> it0 =
				latLngToWt.entrySet().iterator();
		...
		final Iterator<Map.Entry<ParticlePlus, double[]>> it1 =
				prtclPluSToWt.entrySet().iterator();
		final ArrayList<MassFinder.Item> items =
				new ArrayList<>(nDistinctPrtclPlusS);
		for (int k = 0; k < nDistinctPrtclPlusS; ++k) {
			...
			items.add(new MassFinder.Item(xy[0], xy[1], wt, objTp, null));
			assert _objTpToSweepWidth
					.get(new IntDouble(objTp)) != null : "FixedAngleOptProblem1";
		}
		...
		final double minTsNmi = PatternUtilStatics
				.roundWithMinimum(PatternUtilStatics._TsInc, _pv.getMinTsNmi());
		if (patternKind.isPsCs()) {
			...
			/**
			 * The following 3 are what we use to get bestScore as big as
			 * possible. The initialization of bestLlrh is one big cell.
			 */
			double[] bestLlrh =
					new double[] { bigMinX, bigMinY, bigMaxX, bigMaxY };
			final int nCellsForBig = 1;
			IndexFinder bestXIndexFinder =
					new IndexFinder(bigMidX, bigXWidth, nCellsForBig);
			IndexFinder bestYIndexFinder =
					new IndexFinder(bigMidY, bigYWidth, nCellsForBig);
			...
			for (int k = 0; k < PvPlacer._NTimesToSplit; ++k) {
				...
				final IntDouble[] splits =
						getSplit(xExtent, yExtent, PvPlacer._NCellsOnBigSide);
				/**
				 * nX is the number of cells + 1, since nX refers to the grid lines
				 * not the middles of the cells.
				 */
				...
				final IndexFinder xIdxFinder =
						new IndexFinder(midX, xCellWidth, nX);
				final IndexFinder yIdxFinder =
						new IndexFinder(midY, yCellWidth, nY);
					...
					for (int xIdx2 = xIdx1; xIdx2 <= xIdxHgh; ++xIdx2) {
						...
						for (int yIdx1 = yIdxLow; yIdx1 <= yIdxHgh; ++yIdx1) {
							...
							for (int yIdx2 = yIdx1; yIdx2 <= yIdxHgh; ++yIdx2) {
								...
								final double[] llrh =
										new double[] { left, low, right, high };
			...
			/**
			 * computePvValue(bestLlrh) might come back null. We just assume that
			 * it won't.
			 */
			...
			final double[] bestLlrhAndScore =
					getBestLlrhAndScoreForSquares(boxLengthNmi, _NInVsLattice);
			...
			for (int iPass = 0; iPass < 2; ++iPass) {
				final double tsNmi0 = iPass == 0 ? minTsNmi : PatternUtilStatics
						.roundWithMinimum(PatternUtilStatics._TsInc, avgSw);
				final int nHalfLaps = (int) Math.floor(Math.sqrt(eplNmi / tsNmi0));
				final double ssTsNmi =
						Math.floor((eplNmi / (nHalfLaps * nHalfLaps)) /
								PatternUtilStatics._TsInc) * PatternUtilStatics._TsInc;
				final double boxLengthNmi = ssTsNmi * nHalfLaps;
				final double[] bestLlrhAndScoreX =
						getBestLlrhAndScoreForSquares(boxLengthNmi, _NInSsLattices);
				if (bestLlrhAndScore == null ||
						bestLlrhAndScoreX[4] > bestLlrhAndScore[4]) {
	...
	/**
	 * Used only for VS and SS; we simply "rattle" squares around in the
	 * allowable area.
	 */
	private double[] getBestLlrhAndScoreForSquares(final double boxLengthNmi,
			final int _nInLattice) {
		...
		if (boxLengthNmi > maxXNmi - minXNmi) {
			xNmis = new double[] { (minXNmi + maxXNmi) / 2d };
		} else {
		...
		if (boxLengthNmi > maxYNmi - minYNmi) {
			yNmis = new double[] { (minYNmi + maxYNmi) / 2d };
		} else {
			...
			final double nmiGap =
					(lastMidpoint - firstMidpoint) / (_nInLattice - 1);
		...
		for (final double midXNmi : xNmis) {
			...
			for (final double midYNmi : yNmis) {
				...
				final double[] llrh = new double[] { left, low, right, high };
				final double score = getScore(llrh);
	...
	private double getScore(final double[] llrh) {
		...
		for (final IntDouble objTpSw : _objTpToSweepWidth.keySet()) {
			...
			final double mass = _massFinder.getMass(left, low, right, high,
					includeLeft, includeLow, includeRight, includeHigh, objTp);
			weightedSw += mass * sw;
			totalMass += mass;
		}
		...
		final double pod =
				_pv.getNftPod(_searchDurationSecs, avgSw, dim0Nmi, dim1Nmi);
	...
	private PvValue computePvValue(final double[] bestLlrh) {
		...
		if (patternKind.isPsCs()) {
			...
			final PvValue pvValue = PvValue.createPvValue(_pv,
					_pv.getPvCstRefSecs(), _pv.getPvRawSearchDurationSecs(),
		...
		if (patternKind.isSs()) {
			...
			final PvValue pvValue =
					PvValue.createPvValue(_pv, _pv.getPvCstRefSecs(),
							_pv.getPvRawSearchDurationSecs(), midLatLng, firstLegDirR,
							/* alongR= */Double.NaN, acrossR * firstTurnRightD);
			return pvValue;
		}
		if (patternKind.isVs()) {
			/**
			 * All we can do is use the center point and play with the firstLegHdg
			 * and firstTurnRight.
			 */
			final PvValue pvValue = PvValue.createPvValue(_pv,
					_pv.getPvCstRefSecs(), _pv.getPvRawSearchDurationSecs(),
					midLatLng, /* firstLegDirR= */Math.toRadians(90d - _firstLegHdg),
					firstTurnRight);
			return pvValue;
		}
		return null;
	...
	private double[] getTwistedXy(final LatLng3 latLng) {
		final TangentCylinder.FlatLatLng flatLatLng =
				_tangentCylinder.convertToMyFlatLatLng(latLng);
	...
	private static IntDouble[] getSplit(final double xExtent,
			final double yExtent, final int nCellsOnBigSide) {
		...
		if (Math.abs(smallCellSizeA - bigCellSize) < Math
				.abs(smallCellSizeB - bigCellSize)) {
		...
		final IntDouble intDouble1 =
				new IntDouble(nCellsOnBigSide + 1, bigCellSize);
		final IntDouble intDouble2 =
				new IntDouble(nCellsOnSmallSide + 1, smallCellSize);
		if (xIsBig) {
			return new IntDouble[] { intDouble1, intDouble2 };
		}
		return new IntDouble[] { intDouble2, intDouble1 };
	}

	public ArrayList<CornersAndWeights> getCornersAndWeights() {
		final ArrayList<CornersAndWeights> cornersAndWeightsS =
				new ArrayList<>();
		...
		for (int k1 = ixLow; k1 <= ixHgh; ++k1) {
			...
			for (int k2 = iyLow; k2 <= iyHgh; ++k2) {
				...
				final double d0 = _massFinder.getMass(xLow, yLow, xHgh, yHgh, true,
						true, true, true, MassFinder._AllItemTypeIds);
				if (d0 > 0d) {
					final HashMap<IntDouble, IntDouble> objTpToMass = new HashMap<>();
					final IntDouble intDouble1 =
							new IntDouble(MassFinder._AllItemTypeIds, d0);
					...
					final Iterator<IntDouble> it =
							_objTpToSweepWidth.keySet().iterator();
					for (int k3 = 0; k3 < nObjTps; ++k3) {
						final IntDouble objTpSweepWidth = it.next();
						final int objTp = objTpSweepWidth._i;
						final double d1 = _massFinder.getMass(xLow, yLow, xHgh, yHgh,
								true, true, true, true, objTp);
						final IntDouble intDouble2 = new IntDouble(objTp, d1);
						objTpToMass.put(intDouble2, intDouble2);
					}
					cornersAndWeightsS
							.add(new CornersAndWeights(corners, objTpToMass));
				}
			}
		}
