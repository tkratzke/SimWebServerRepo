...
public class PvValue implements Comparable<PvValue> {
	...
	/** Standard Comparator. */
	final public static Comparator<? super PvValue> _ByPvGrandOrdinalFirst = new Comparator<>() {
	...
	final public static Comparator<? super PvValue> _ByPvOrdWithinPvSeq = new Comparator<>() {
		...
		public int compare(final PvValue pvValue0, final PvValue pvValue1) {
			...
			final PatternVariable pv0 = pvValue0.getPv();
			final PatternVariable pv1 = pvValue1.getPv();
			final int compareValue = PatternVariable._ByOrdWithinPvSeq.compare(pv0, pv1);
			if (compareValue != 0) {
				return compareValue;
			}
	...
	/**
	 * If and only if _myStyle == null, this is a TrackLine _pv is a TrackLine PV.
	 */
	...
	/**
	 * To avoid re-computing this for each particlette that is within the TsBox, we
	 * store it here. Note that we use pos = poc * pod only for optimization, so we
	 * store only the viz2 LrcSets.
	 */
	...
	public PvValue(final PatternVariable pv, final MyStyle myStyle) {
		...
		if (onMars) {
			...
			/**
			 * We need a rawSearchKtsForOnMars that will use up all the time for the tiny
			 * onMars pattern. Start by computing the length of _myStyle's pattern. For
			 * OnMars, _myStyle's along and across are so small that, together with
			 * minTsNmi, the epl computed is very small.
			 */
			final double eplNmiForOnMars = _myStyle.computeEplNmi(rawSearchKts, minTsNmi);
			...
			if (patternKind.isPsCs()) {
				final double lenNmi = _myStyle.computeLengthNmi(rawSearchKtsForOnMars, minTsNmi);
				final double widNmi = _myStyle.computeWidthNmi(rawSearchKtsForOnMars, minTsNmi);
				simLibPattern = new SimLibPattern(rawSearchKtsForOnMars, cstRefSecs, /* baseEqualsUnix= */false,
						searchDurationSecs, centerLat, centerLng, orntn, //
						/* firstTurnRight= */true, //
						/* minTsNmi= */minTsNmi, /* fxdTsNmi= */minTsNmi, excBufferNmi, lenNmi, widNmi, /* ps= */true, //
				...
				simLibPattern = new SimLibPattern(rawSearchKtsForOnMars, cstRefSecs, /* baseEqualsUnix= */false,
						searchDurationSecs, centerLat, centerLng, orntn, //
						/* firstTurnRight= */true, //
						/* minTsNmi= */minTsNmi, /* fxdTsNmi= */minTsNmi, excBufferNmi, lenNmi, /* widNmi= */Double.NaN,
						/* ps= */true, //
						MotionType.GREAT_CIRCLE.getId(), /* expandSpecsIfNeeded= */true);
				...
				simLibPattern = new SimLibPattern(rawSearchKtsForOnMars, cstRefSecs, /* baseEqualsUnix= */false,
						searchDurationSecs, centerLat, centerLng, orntn, //
						/* firstTurnRight= */true, //
						/* minTsNmi= */minTsNmi, /* fxdTsNmi= */minTsNmi, excBufferNmi, /* lenNmi= */Double.NaN,
			...
			_sortie = _pv.computeSortie(sphericalTimedSegs, creepHdg, tsNmiForSingleLeg, /* onMars= */true);
			_looseSpecLatLngs = sphericalTimedSegs.getLooseLatLngs(SphericalTimedSegs.LoopType.SPEC);
			_tightTsLatLngs = sphericalTimedSegs.getTightLatLngs(SphericalTimedSegs.LoopType.TS);
			_looseTsLatLngs = sphericalTimedSegs.getLooseLatLngs(SphericalTimedSegs.LoopType.TS);
			_tightExcLatLngs = sphericalTimedSegs.getTightLatLngs(SphericalTimedSegs.LoopType.EXC);
		...
		final double tsNmiForSingleLeg;
		if (patternKind.isPsCs()) {
			final double lenNmi = myStyle.computeLengthNmi(rawSearchKts, minTsNmi);
			final double widNmi = myStyle.computeWidthNmi(rawSearchKts, minTsNmi);
			tsNmiForSingleLeg = Math.abs(myStyle.computeSgndAcrossNmi(rawSearchKts, minTsNmi));
			final SimLibPattern ladderPattern = new SimLibPattern(rawSearchKts, cstRefSecs, /* baseEqualsUnix= */false,
					searchDurationSecs, //
			...
			final double lenNmi = myStyle.computeLengthNmi(rawSearchKts, minTsNmi);
			final SimLibPattern simLibPattern = new SimLibPattern(rawSearchKts, cstRefSecs, /* baseEqualsUnix= */false,
					searchDurationSecs, //
					centerLat, centerLng, orntn, firstTurnRight, //
					minTsNmi, /* fxdTsNmi= */Double.NaN, excBufferNmi, //
		...
		final double creepHdg = _myStyle.computeCreepHdg(rawSearchKts, minTsNmi);
		_sortie = _pv.computeSortie(sphericalTimedSegs, creepHdg, tsNmiForSingleLeg, /* onMars= */false);
		_looseSpecLatLngs = sphericalTimedSegs.getLooseLatLngs(SphericalTimedSegs.LoopType.SPEC);
		_tightTsLatLngs = sphericalTimedSegs.getTightLatLngs(SphericalTimedSegs.LoopType.TS);
		_looseTsLatLngs = sphericalTimedSegs.getLooseLatLngs(SphericalTimedSegs.LoopType.TS);
		_tightExcLatLngs = sphericalTimedSegs.getTightLatLngs(SphericalTimedSegs.LoopType.EXC);
	...
	public PvValue(final SimCase simCase, final PatternVariable pv, final Element initPvValueElt,
		...
		if (isTrackLine) {
			_myStyle = null;
			final double[] creepHdgAndTs = ModelReader.readCreepHdgAndTs(simCase, initPvValueElt, stringPluses);
			/**
			 * Note; creepDirectionIn is used only for a single leg. In that case, we assume
			 * that it's a ladder pattern and we actually get the creep direction from the
			 * heading of the 1st leg, but use creepDirectionIn to figure out if the "creep"
			 * is to the left or right of the single leg.
			 */
			...
			final ElementIterator childIterator = new ElementIterator(initPvValueElt);
			/** Add waypoints. */
			while (childIterator.hasNextElement()) {
				...
				if ("WAYPOINT".equals(childTag)) {
					try {
						final LatLng3 latLng = ModelReader.readLatLng(simCase, child, stringPluses);
						final long refSecs = ModelReader.readDtg(simCase, child, "dtg", stringPluses);
						if (recentLatLng != null) {
							_sortie.addLegIfNotVacuous(recentLatLng, latLng, recentRefSecs, refSecs);
						}
						recentLatLng = latLng;
			...
			/**
			 * Flesh out this TrackLine PvValue. We need the viz1 and viz2 for _sortie to
			 * work.
			 */
			...
			if (psCsInfo != null || ssInfo != null || vsInfo != null) {
				...
				final SphericalTimedSegs sphericalTimedSegs;
				if (psCsInfo != null) {
					sphericalTimedSegs = psCsInfo.computeSphericalTimedSegs(cst, searchDurationSecs, bufferNmi);
				} else if (ssInfo != null) {
					sphericalTimedSegs = ssInfo.computeSphericalTimedSegs(cst, searchDurationSecs, bufferNmi);
				} else {
					/** vsi != null */
					sphericalTimedSegs = vsInfo.computeSphericalTimedSegs(cst, searchDurationSecs, bufferNmi);
				}
				_looseSpecLatLngs = sphericalTimedSegs.getLooseLatLngs(LoopType.SPEC);
				_tightTsLoop = sphericalTimedSegs.getTightTsLoop();
				_tightTsLatLngs = sphericalTimedSegs.getTightLatLngs(SphericalTimedSegs.LoopType.TS);
		...
		if (standAlone) {
			...
			searchDurationSecs = ModelReader.getDurationSecs(simCase, pvElt, "duration", stringPluses);
		} else {
			/**
			 * Get _cstRefSecs and _estRefSecs from this element. Even onMars requires cst
			 * and duration.
			 */
			final String pvCstString = initPvValueElt.getAttribute("cst");
			cstRefSecs = TimeUtilities.dtgToRefSecs(pvCstString);
			searchDurationSecs = ModelReader.getDurationSecs(simCase, initPvValueElt, "duration", stringPluses);
		}
		/** Learn about Mars. */
		boolean onMars = false;
		try {
			onMars = ModelReader.getBoolean(simCase, initPvValueElt, "onMars", /* default= */false, stringPluses);
		} catch (final ReaderException e) {
		}
		...
		if (onMars) {
			...
			try {
				final double rawSearchKts = _pv.getRawSearchKts();
				/**
				 * Not onMars. center, orntn, and firstTurnRight are all mandatory for LP, VS,
				 * and SS.
				 */
				LatLng3 center = null;
				double orntn = Double.NaN;
				boolean firstTurnRight = true;
				final double centerLat = ModelReader.getDouble(simCase, initPvValueElt, "centerPointLat", "",
						Double.NaN, stringPluses);
				final double centerLng = ModelReader.getDouble(simCase, initPvValueElt, "centerPointLng", "",
						Double.NaN, stringPluses);
				final LatLng3 center0 = LatLng3.getLatLngB(centerLat, centerLng);
				center = PatternUtilStatics.DiscretizeLatLng(center0);
				final double orntn0 = ModelReader.getDouble(simCase, initPvValueElt, "orientation",
						" degrees clockwise from north", Double.NaN, stringPluses);
				orntn = PatternUtilStatics.DiscretizeOrntn(orntn0);
				firstTurnRight = ModelReader.getBoolean(simCase, initPvValueElt, "firstTurnRight", true, stringPluses);

				/** Read in the rest of the data. */
				if (patternKind.isPsCs()) {
					final boolean readInPs;
					final String psOrCsBooleanString = initPvValueElt.getAttribute("PS");
					if (psOrCsBooleanString == null || psOrCsBooleanString.length() == 0) {
						String psOrCsString = null;
						try {
							psOrCsString = ModelReader.getStringNoDefault(simCase, initPvValueElt, "pathType",
									stringPluses);
						} catch (final ReaderException e) {
							e.printStackTrace();
							MainRunner.HandleFatal(simCase, new RuntimeException("Bad pathType value."));
						}
						readInPs = psOrCsString == null ? true : psOrCsString.compareToIgnoreCase("PS") == 0;
					} else {
						final Boolean bb = StringUtilities.getBoolean(psOrCsBooleanString);
					...
					final double specLenNmi0 = ModelReader.getDouble(simCase, initPvValueElt, "specLength", " NM",
							Double.NaN, stringPluses);
					final double specWidNmi0 = ModelReader.getDouble(simCase, initPvValueElt, "specWidth", " NM",
							Double.NaN, stringPluses);
					if (specLenNmi0 > 0d && specWidNmi0 > 0d) {
						...
						/**
						 * They're giving us the rounded TsBox. Use the reverse mechanism to get the
						 * SpecBox.
						 */
						final double tsLenNmi = ModelReader.getDouble(simCase, initPvValueElt, "length", " NM",
								Double.NaN, stringPluses);
						final double tsWidNmi = ModelReader.getDouble(simCase, initPvValueElt, "width", " NM",
								Double.NaN, stringPluses);
						...
						/**
						 * Use DiscLpTsBoxAdjuster' inversion ctor to find some length/width we can use
						 * to get the rounded tsLength/tsWidth we just read in.
						 */
						final double eplNmi = rawSearchKts * (searchDurationSecs / 3600d)
								* PatternUtilStatics._EffectiveSpeedReduction;
						final DiscreteLpSpecToTs discLpTsBoxAdjuster = new DiscreteLpSpecToTs(eplNmi, tsAlongNmi,
								tsAcrossNmi);
						if (discLpTsBoxAdjuster.getAcross() > 0d) {
							...
							/**
							 * Couldn't reverse it. "Forward" with the given data, and allow adjustments.
							 * Just get something.
							 */
							final DiscreteLpSpecToTs discLpTsBoxAdjuster2 = new DiscreteLpSpecToTs(eplNmi,
									_pv.getMinTsNmi(), /* fxdTsNmi= */Double.NaN, tsAlongNmi, tsAcrossNmi,
					...
					if ((specLenNmi <= 0d)) {
						MainRunner.HandleFatal(simCase, new RuntimeException("Bad BOX dimensions."));
					}
					myStyle0 = new MyStyle(patternKind, cstRefSecs, searchDurationSecs, center, orntn, firstTurnRight,
							specLenNmi, specWidNmi, ps, /* tsNmi= */Double.NaN);
				} else if (patternKind.isSs()) {
					final double specLenNmi;
					...
					if (specLenNmi0 > 0d) {
						...
						final double tsLenNmi = ModelReader.getDouble(simCase, initPvValueElt, "length", " NM",
								Double.NaN, stringPluses);
						/**
						 * Use DiscSsTsBoxAdjuster' inversion ctor to find some length we can use to get
						 * the rounded tsLength we just read in.
						 */
						final double eplNmi = rawSearchKts * (searchDurationSecs / 3600d)
								* PatternUtilStatics._EffectiveSpeedReduction;
						final DiscreteSsSpecToTs discSsTsBoxAdjuster = new DiscreteSsSpecToTs(eplNmi,
								/* acrossNmi= */tsLenNmi);
						if (discSsTsBoxAdjuster.getAcross() > 0d) {
							 ...
							 * Just get something.
							 */
							final DiscreteSsSpecToTs discSsTsBoxAdjuster2 = new DiscreteSsSpecToTs(eplNmi,
									_pv.getMinTsNmi(), /* fxdTsNmi= */Double.NaN, /* acrossNmi= */tsLenNmi,
									/* adjustSpecsIfNeeded= */true);
					...
					myStyle0 = new MyStyle(patternKind, cstRefSecs, searchDurationSecs, center, orntn, firstTurnRight,
							specLenNmi, /* widthNmi= */Double.NaN, /* ps= */true, /* tsNmi= */Double.NaN);
				} else if (patternKind.isVs()) {
					/** Nothing else to read. */
					myStyle0 = new MyStyle(_pv, cstRefSecs, searchDurationSecs, center, orntn, /* alongR= */Double.NaN,
							/* sgndAcrossR= */firstTurnRight ? 1d : -1d, /* inputIsRadians= */'n');
				}
			} catch (final ReaderException e) {
				MainRunner.HandleFatal(simCase, new RuntimeException("Bad BOX tag."));
			}
		}
		final MyStyle myStyle = myStyle0;

		/**
		 * We now have myStyle. Create a PvValue from it and copy the fields over.
		 */
	...
	public static PvValue createPvValue(final PatternVariable pv, final long cstRefSecs, final int searchDurationSecs,
			final LatLng3 center, final double firstLegDirR0, final double alongR0, final double sgndAcrossR0) {
		...
		if (patternKind.isPsCs()) {
			...
			myStyle = new MyStyle(pv, cstRefSecs, searchDurationSecs, center, firstLegDirR, alongR, sgndAcrossR,
					/* inputIsRadiansChar= */'y');
		} else if (patternKind.isSs()) {
			firstLegDirR = firstLegDirR0;
			myStyle = new MyStyle(pv, cstRefSecs, searchDurationSecs, center, firstLegDirR, /* alongR= */Double.NaN,
					sgndAcrossR0, /* inputIsRadiansChar= */'y');
		} else {
			myStyle = null;
		}
		final PvValue pvValue = new PvValue(pv, myStyle);
	...
	public static PvValue createPvValue(final PatternVariable pv, final long cstRefSecs,
			final int rawSearchDurationSecs, final LatLng3 center, final double firstLegDirR,
			final boolean firstTurnRight) {
		final MyStyle myStyle = new MyStyle(pv, cstRefSecs, rawSearchDurationSecs, center, firstLegDirR,
				/* alongNmi= */Double.NaN, /* sgndAcrossNmi= */firstTurnRight ? 1d : -1d, /* inputIsRads= */'y');
		final PvValue pvValue = new PvValue(pv, myStyle);
		return pvValue;
	}

	private static SimCaseManager.SimCase getSimCase(final PatternVariable pv) {
		final PlannerModel plannerModel = pv.getPlannerModel();
	...
	/**
	 * We need this for pos = poc * pod, and we use that only during optimization,
	 * so we always use viz2.
	 */
	private HashMap<Integer, Double> computeObjectTypeToPod() {
		final double tsNmi;
		if (_myStyle != null) {
			tsNmi = _myStyle.computeTsNmi(_pv.getRawSearchKts(), _pv.getMinTsNmi());
		} else {
	...
	private HashMap<Integer, Double> computeObjectTypeToPod(final double tsNmi) {
		final HashMap<Integer, Double> objectTypeToPod = new HashMap<>();
		final PlannerModel plannerModel = _pv.getPlannerModel();
		final CoverageToPodCurve coverageToPodCurve = plannerModel.getCoverageToPodCurve();
		...
		for (final Map.Entry<Integer, LrcSet> entry : viz2LrcSets.entrySet()) {
			...
			final double sweepWidth = lrcSet == null ? 0d : lrcSet.getSweepWidth();
			if (sweepWidth > 0d && tsNmi > 0d) {
				final double coverage = sweepWidth / tsNmi;
				objectTypeToPod.put(objectType, coverageToPodCurve.coverageToPod(coverage));
			}
		}
	...
	/**
	 * This is only used during optimization (pos = poc * poc). Hence, we always use
	 * viz2.
	 */
	...
	public static int ComparePvValueArrays(final PvValue[] pvValueArray0, final PvValue[] pvValueArray1) {
	...
	private static Loop3 computeBufferedCcwCvxHull(final MyLogger logger, final GreatCircleArc[] gcas,
			final double bufferNmi) {
		final int nGcas = gcas.length;
		final TreeSet<LatLng3> distinctLatLngs = new TreeSet<>(LatLng3._ByLatThenLng);
		for (int k = 0; k < nGcas; ++k) {
			final GreatCircleArc gca = gcas[k];
			final LatLng3[] ccwBox = Loop3Statics.createCcwBox(gca.getLatLng0(), gca.getLatLng1(), bufferNmi, bufferNmi,
					bufferNmi, bufferNmi);
			distinctLatLngs.addAll(Arrays.asList(ccwBox));
		}
		final Loop3 cvxHull = Loop3Statics.getConvexHull(logger, distinctLatLngs, /* cw= */false);
		final List<LatLng3> separated = cvxHull.cullSmallEdges(0.005);
		if (separated.size() == cvxHull.getNLatLngs()) {
			return cvxHull;
		}
		final LatLng3[] latLngArray = separated.toArray(new LatLng3[separated.size()]);
		final Loop3 newLoop = Loop3.getLoop(logger, /* id= */0, /* subId= */0, cvxHull.getFlag(), /* ancestorId= */-1,
				latLngArray, /* logChanges= */false, /* debug= */false);
		if (newLoop.getNLatLngs() != latLngArray.length || latLngArray.length < 4) {
			return cvxHull;
		}
		return newLoop;
	}
	...
	public double[] getProportionInAndNftPFail(final ParticleIndexes prtclIndxs) {
		if (onMars()) {
			return new double[] {
					0d, 1d
			};
		...
		final long[] fencePosts = CombinatoricTools.getFenceposts(cstRefSecs, endRefSecs,
				ParticlesManager._NumberOfStages);
		final ParticlesManager particlesManager = _pv.getPlannerModel().getPlanner().getParticlesManager();
		for (final long fencePost : fencePosts) {
			final LatLng3 latLng = particlesManager.getPosition(fencePost, prtclIndxs);
			if (tsBoxContains(latLng)) {
				++nContained;
				final int objectType = particlesManager.getObjectType(fencePost, prtclIndxs);
				totalOfPFails += 1d - getPod(objectType);
			} else {
				totalOfPFails += 1d;
			}
		}
		final double propIn = (double) nContained / ParticlesManager._NumberOfStages;
	...
	public double getCos() {
		if (_myStyle == null) {
			final PsCsInfo psCsInfo = _sortie.getPsCsInfo();
			if (psCsInfo != null) {
				final double cos = MathX.cosX(Math.toRadians(90d - psCsInfo.getFirstLegHdg()));
				return cos;
			}
			final VsInfo vsInfo = _sortie.getVsInfo();
			if (vsInfo != null) {
				final double cos = MathX.cosX(Math.toRadians(90d - vsInfo.getFirstLegHdg()));
				return cos;
			}
			final SsInfo ssInfo = _sortie.getSsInfo();
			if (ssInfo != null) {
				final double cos = MathX.cosX(Math.toRadians(90d - ssInfo.getFirstLegHdg()));
				return cos;
			}
	...
	public double getSin() {
		if (_myStyle == null) {
			final PsCsInfo psCsInfo = _sortie.getPsCsInfo();
			if (psCsInfo != null) {
				final double sin = MathX.sinX(Math.toRadians(90d - psCsInfo.getFirstLegHdg()));
				return sin;
			}
			final VsInfo vsInfo = _sortie.getVsInfo();
			if (vsInfo != null) {
				final double sin = MathX.sinX(Math.toRadians(90d - vsInfo.getFirstLegHdg()));
				return sin;
			}
			final SsInfo ssInfo = _sortie.getSsInfo();
			if (ssInfo != null) {
				final double sin = MathX.sinX(Math.toRadians(90d - ssInfo.getFirstLegHdg()));
				return sin;
			}
			/** We have no way of specifying an angle for an irregular sortie. */
			return Double.NaN;
	...
	public double getCreepHdg() {
		...
		if (_pv.getPatternKind().isPsCs()) {
			...
			final double alongHdg = Math.toDegrees(Constants._PiOver2 - myStyle.getFirstLegDirR());
	...
	public int computeNSearchLegs() {
		...
		return _myStyle.computeNSearchLegs(_pv.getRawSearchKts(), _pv.getMinTsNmi());
	}
	...
	public LatLng3[] getLooseLatLngArray(final SphericalTimedSegs.LoopType loopType) {
	...
	public LatLng3[] getTightLatLngArray(final SphericalTimedSegs.LoopType loopType) {
		if (loopType == SphericalTimedSegs.LoopType.SPEC) {
			return null;
		}
		if (loopType == SphericalTimedSegs.LoopType.TS) {
	...
	/**
	 * Just for convenience to make it easy to construct a PvValue to look up a Pv
	 * in a list or array of PvValues.
	 */
	public PvValue(final PatternVariable pv) {
		_pv = pv;
		_looseSpecLatLngs = _tightTsLatLngs = _looseTsLatLngs = _tightExcLatLngs = null;
		_tightTsLoop = null;
		_tightExcCcwGcas = null;
		_sortie = null;
		_myStyle = null;
	...
	public String getExcLoopString() {
		...
		final String colorName = ColorUtils.getColorName(ColorUtils.ColorGrouping.BROWN, iPv);
		final String[] colorNames = new String[] {
				colorName
		};
		final int numberEdgeInc = iPv + 1;
		final String s = ccwLoop.getXmlDump(numberEdgeInc, colorNames, /* GcaFilter= */null);
		return s;
	}
	...
	public int getSearchDurationSecs() {
		if (_myStyle == null) {
			final List<Sortie.Leg> legs = _sortie.getDistinctInputLegs();
			final int nLegs = legs.size();
			return (int) (legs.get(nLegs - 1).getLegRefSecs1() - legs.get(0).getLegRefSecs0());
		}
	...
	public double getEplNmi() {
		...
		final double eplNmi = _myStyle.computeEplNmi(_pv.getRawSearchKts(), _pv.getMinTsNmi());
		return eplNmi;
	}
	...
	public double getTsSqNmi() {
		if (_myStyle != null) {
			return _myStyle.computeTsSqNmi(_pv.getRawSearchKts(), _pv.getMinTsNmi());
		}
	...
	public String getString() {
		if (_myStyle == null) {
			return String.format(
					...
					_sortie.getDistinctGcas().length, _sortie.getTtlLegsNmi(), _sortie.getFirstLegHdg(), //
					_sortie.getTsNmiIn(), _sortie.getTsNmi(), //
					_sortie.getCreepHdgIn(), _sortie.getCreepHdg() //
			);
		}
		return String.format("%s", _myStyle.getString(_pv.getRawSearchKts(), _pv.getMinTsNmi()));
	}
