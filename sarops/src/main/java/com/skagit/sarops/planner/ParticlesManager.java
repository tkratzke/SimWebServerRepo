...
public class ParticlesManager {
	...
	/**
	 * The worst possible variance is when 1/2 of the particles have a pos of 0 and
	 * the other half have a pos of 1. In this case, the true variance is 0.25.
	 */
	...
	/**
	 * We start by implementing a "friend" mechanism. The access will always be
	 * buried in posFunction's private field so only posFunction can get at these.
	 */
	...
	public class AccessToKeyPrivates {
		...
		public ParticlesAndPriors getSampleParticlesAndPrior(final ParticleIndexes[] prtclIndxsS0,
				final double[] priors0, final int nInSample, final Randomx r) {
			final ParticleIndexes[] prtclIndxsS1;
			final double[] priors1;
			if (prtclIndxsS0 == null || priors0 == null || prtclIndxsS0.length != priors0.length) {
	...
	public static class Specs {
		...
		public Specs(final boolean includeLandedParticles, final boolean includeAdriftParticles,
				final long firstRefSecsOfInterest, final long lastRefSecsOfInterest,
				final HashSet<Integer> viz1ObjectTypeIds, final HashSet<Integer> viz2ObjectTypeIds) {
		...
		private static boolean contains(final int objectTypeId, final int[] array) {
	...
	/**
	 * More double arrays. "Priors" means relative probability AFTER simModel's
	 * completed searches. "InitialPriors" means prior BEFORE simModel's completed
	 * searches. "OldPFails" means the pFails of simModel's completed searches. All
	...
	/**
	 * Selected; those for which some Pv (frozen or not) is looking for, and pass
	 * the L/A test.
	 */
	...
	/**
	 * For Optn and sampling; _particlesForOptn includes only those that can be seen
	 * by some Pv that is not permanently frozen, and pass the L/A test. We also
	 * take into account the frozens when computing _priorsForOptn, and thereafter,
	 * any PosFunction that is created for optimization, ignores the permanently
	 * frozen PttrnVbls.
	 */
	...
	public static ParticlesManager createParticlesManager(final SimCaseManager.SimCase simCase,
			final ParticlesFile particlesFile, final Specs specs) {
		try {
			final ParticlesManager particlesManager = new ParticlesManager(simCase, particlesFile, specs);
			final ParticleIndexes[] selectedPrtclIndxsS = particlesManager._selectedParticles;
			final int nSelected = selectedPrtclIndxsS == null ? 0 : selectedPrtclIndxsS.length;
			if (nSelected == 0) {
	...
	public ParticleIndexes.ParticleIndexesState computePrtclIndxsState(final ParticleIndexes prtclIndxs,
			final long refSecs) {
		final LatLng3 latLng = _particlesFile.getLatLng(refSecs, prtclIndxs);
		if (latLng == null) {
			return null;
		}
		final int objectType = _particlesFile.getSotId(refSecs, prtclIndxs);
		final long landingRefSecs = _particlesFile.getLandingRefSecs(prtclIndxs);
		final boolean isLanded = landingRefSecs >= 0 && landingRefSecs <= refSecs;
		return prtclIndxs.new ParticleIndexesState(latLng, refSecs, objectType, isLanded);
	}
	...
	private boolean isSelected(final ParticleIndexes prtclIndxs) {
		...
		final int objectTypeId1 = _particlesFile.getSotId(firstSearchRefSecs, prtclIndxs);
		if (!_specs.viz2(objectTypeId1)) {
			final int objectTypeId2 = _particlesFile.getSotId(lastSearchRefSecs, prtclIndxs);
	...
	/**
	 * With our ParticlesFile, a particle passes the landed/Adrift test if it passes
	 * it as of the first search time.
	 */
	private boolean passesLandedAdriftTest(final ParticleIndexes prtclIndxs) {
		final long landingRefSecs = _particlesFile.getLandingRefSecs(prtclIndxs);
		final boolean isAdrift = landingRefSecs < 0 || landingRefSecs > _specs._firstRefSecsOfInterest;
		...
		private SetOfStrata() {
			_strata = new TreeSet<>(new Comparator<Stratum>() {
				...
				public int compare(final Stratum o1, final Stratum o2) {
					...
					return o1._distressType < o2._distressType ? -1 : (o1._distressType > o2._distressType ? 1 : 0);
				}
			});
		}

		private void add(final ParticleIndexes prtclIndxs, final int iScenario, final int distressType,
				final double weight) {
	...
	private static class Stratum {
		...
		private void add(final ParticleIndexes prtclIndxs, final double weight) {
			final StratumMember stratumMember = new StratumMember(prtclIndxs, weight);
	...
	private static class StratumMember {
		...
		private StratumMember(final ParticleIndexes prtclIndxs, final double weight) {
			_prtclIndxs = prtclIndxs;
			_weight = weight;
		}
	}

	private ParticlesManager(final SimCaseManager.SimCase simCase, final ParticlesFile particlesFile, final Specs specs)
			throws Exception {
		_specs = specs;
		...
		final long[] stageRefSecsS = CombinatoricTools.getFenceposts(firstRefSecsOfInterest, lastRefSecsOfInterest,
				_NumberOfStages);
		for (int iStage = 0; iStage < _NumberOfStages; ++iStage) {
			stageMarkers[iStage] = findClosestRefSecs(refSecsS, stageRefSecsS[iStage]);
		}
		if (stageMarkers[0] == null || stageMarkers[_NumberOfStages - 1] == null) {
			final String time1String = TimeUtilities.formatTime(firstRefSecsOfInterest, true);
			final String time2String = TimeUtilities.formatTime(lastRefSecsOfInterest, true);
			SimCaseManager.err(simCase, "Insufficient simulation data at " + time1String + " or " + time2String);
		}
		...
		/**
		 * Compute nAll, nSelected and nLandedAdrift; these are automatically everything
		 * for a non-planner problem.
		 */
		...
		for (int iScenario = 0; iScenario < nScenarii; ++iScenario) {
			for (int iParticle = 0; iParticle < nParticlesPerScenario; ++iParticle) {
				...
				final boolean isSelected = isRealPlannerProblem ? isSelected(prtclIndxs) : true;
				if (isSelected) {
					final int objectTypeId1 = _particlesFile.getSotId(_specs._firstRefSecsOfInterest,
							prtclIndxs);
					if (_specs.viz2(objectTypeId1)) {
						selectedObjectTypeIdSet.add(objectTypeId1);
					}
					final int objectTypeId2 = _particlesFile.getSotId(_specs._lastRefSecsOfInterest, prtclIndxs);
					if (_specs.viz2(objectTypeId2)) {
						selectedObjectTypeIdSet.add(objectTypeId2);
					}
					++nSelected;
				}
				final boolean passesLandedAdrift = isRealPlannerProblem ? passesLandedAdriftTest(prtclIndxs) : true;
		...
		if (isRealPlannerProblem && nSelected == 0) {
			SimCaseManager.err(simCase, "No particles selected for optimization problem.  Quitting.");
			...
			/** _landedAdrift. */
			_priorsGivenLandedAdrift = _initPriorsOfLandedAdrift = _oldPFailsOfLandedAdrift = null;
			/** ObjectTypes. */
		...
		for (int iScenario = 0; iScenario < nScenarii; ++iScenario) {
			for (int iParticle1 = 0; iParticle1 < nParticlesPerScenario; ++iParticle1) {
				final ParticleIndexes prtclIndxs = ParticleIndexes.getStandardOne(model, iScenario, iParticle1);
				final int overallIndex = prtclIndxs.getOverallIndex();
				_allParticles[overallIndex] = prtclIndxs;
				/**
				 * TMK!! Note 1: _allPriors should be getProbabilities[refSecsForPrior] and not
				 * multiplying in cumPFail.
				 */
				final double initPrior = _particlesFile.getProbability(refSecsS[0], prtclIndxs);
				final double cumPFail = _particlesFile.getCumPFail(refSecsForPrior, prtclIndxs);
		...
		for (final ParticleIndexes prtclIndxs : _allParticles) {
			final int thisObjectType = _particlesFile.getSotId(refSecsS[0], prtclIndxs);
			allObjectTypes.add(thisObjectType);
			allObjectTypes.add(_particlesFile.getSotId(refSecsS[nRefSecsS - 1], prtclIndxs));
		}
		...
		if (isRealPlannerProblem) {
			...
			/**
			 * Normalizing _allInitPriors is unnecessary; _allPriors, since it has the
			 * pFails from Sim's completed searches multiplied in, is necessary.
			 */
			...
			if (nDiscarded1 == 0) {
				...
				for (int iScenario = 0; iScenario < nScenarii; ++iScenario) {
					for (int iParticle = 0; iParticle < nParticlesPerScenario; ++iParticle) {
						final ParticleIndexes prtclIndxs = ParticleIndexes.getStandardOne(model, iScenario, iParticle);
						/** TMK!! See Note 1. */
						final double origPrior = _particlesFile.getProbability(refSecsS[0], prtclIndxs);
						final double cumPFail = _particlesFile.getCumPFail(refSecsForPrior, prtclIndxs);
		...
		final long[] fencePostsInRefSecsS = CombinatoricTools.getFenceposts(firstFencePost, lastFencePost,
				_NumberOfStages);
		for (int indexForSelected = 0; indexForSelected < nSelected; ++indexForSelected) {
			final ParticleIndexes prtclIndxs = _selectedParticles[indexForSelected];
			final double prior = _priorsGivenSelected[indexForSelected];
			for (int iStage = 0; iStage < _NumberOfStages; ++iStage) {
				final long refSecs = fencePostsInRefSecsS[iStage];
				final ParticleIndexes.ParticleIndexesState prtclIndxsState = prtclIndxs.refSecsToPrtclIndxsState(this,
						refSecs);
		...
		for (int iScenario = 0; iScenario < nScenarii; ++iScenario) {
			...
			for (int sotOrd = 0; sotOrd < nSearchObjectTypes; ++sotOrd) {
				...
				final SearchObjectType searchObjectType = model.getSotFromOrd(sotOrd);
	...
	public void setOptnPrtclsAndPriors(final Planner planner) {
		...
		/**
		 * Compute the selected particles that are looked at by some Pv that is not
		 * permanently frozen. Multiply in the frozen pFails.
		 */
		...
		for (int kFloater = 0; kFloater < nFloaters; ++kFloater) {
			...
			final Set<Integer> theseViz2ObjectTypes = pv.getViz2LrcSets().keySet();
		...
		for (int kSelected = 0; kSelected < nSelected; ++kSelected) {
			...
			final int objectType = _particlesFile.getSotId(refSecsForPrior, prtclIndxs);
		...
		final PvValue[] permanentlyFrozenPvValues = plannerModel.getPermanentlyFrozenPvValues();
		final int nPermanentlyFrozenPvValues = permanentlyFrozenPvValues == null ? 0 : permanentlyFrozenPvValues.length;
		...
		if (nForOptn == nSelected && nPermanentlyFrozenPvValues == 0) {
			...
			for (int kFrozen = 0; kFrozen < nPermanentlyFrozenPvValues; ++kFrozen) {
				final PvValue pvValue = permanentlyFrozenPvValues[kFrozen];
				final DetectValues[] detectValuesArray = pFailsCache.getDetectValuesArray(planner, forOptnOnly,
						pFailType, _particlesForOptn, 0, nForOptn, pvValue);
				for (int kForOptn = 0; kForOptn < nForOptn; ++kForOptn) {
					_priorsForOptn[kForOptn] *= detectValuesArray[kForOptn]._aiftPFail;
				}
			}
		}

		final double ttlWt0 = NumericalRoutines.normalizeWeights(_priorsForOptn);
		if (ttlWt0 == 0d) {
			_particlesForOptn = new ParticleIndexes[0];
			_priorsForOptn = new double[0];
			_usingStratified = false;
		...
		for (int kForOptn = 0; kForOptn < nForOptn; ++kForOptn) {
			...
			final int objTp = _particlesFile.getSotId(refSecsForPrior, prtclIndxs);
			...
			if (incumbent != null) {
				final IntDouble replacement = new IntDouble(objTp, incumbent._d + wt);
		...
		/**
		 * Construct the sample, stratified or not. Note that, for consistency's sake
		 * across runs, we always use the same seed. Hence, our optimization
		 * PosFunctions, which depend on sampleParticles, will be consistent from one
		 * run to the next.
		 */
		final Randomx r = new Randomx(/* useCurrentTimeMs= */false);
		/**
		 * If and only if the following 2 arrays do not end up null, we are using
		 * stratified.
		 */
		 ...
		 * worthwhile.
		 */
		final SimGlobalStrings simGlobalStrings = planner.getSimCase().getSimGlobalStrings();
		final int minNStrataForStratified = simGlobalStrings.getMinNStrataForStratified();
		if (nStrata >= minNStrataForStratified) {
			...
			final double nForFirstStratum = simGlobalStrings.getNForFirstStratum();
			final int targetN = (int) Math.ceil(factor * nForFirstStratum);
			final StratifiedSample stratifiedSample = new StratifiedSample(inWts, targetN, r);
			...
			for (int k = 0; k < nInSample; ++k) {
				final double[] trio = stratifiedSample.getStratumEntryWeightOfChosen(k);
		...
		if (sampleParticles != null) {
			...
			final int maxSampleSize = (int) Math.ceil(_WorstPossibleSigSq / (maxSigma * maxSigma));
	...
	public static class TimeMarker {
		...
		TimeMarker(final int lowerIndex, final long lowerRefSecs, final long refSecs, final long upperRefSecs) {
	...
	public static TimeMarker findClosestRefSecs(long[] refSecsS, final long refSecs) {
		...
		if (nRefSecsS > 1 && refSecsS[0] > refSecsS[nRefSecsS - 1]) {
			...
			return new TimeMarker(nRefSecsS - 1 - glbIndex, upperRefSecs, refSecs, lowerRefSecs);
		}
		final int glbIndex = CombinatoricTools.getGlbIndex(refSecsS, refSecs);
		if (glbIndex == -1) {
			return null;
	...
	public LatLng3 getPosition(final long refSecs, final ParticleIndexes prtclIndxs) {
		final LatLng3 latLng = _particlesFile.getLatLng(refSecs, prtclIndxs);
		return latLng;
	}

	public int getObjectType(final long refSecs, final ParticleIndexes prtclIndxs) {
		return _particlesFile.getSotId(refSecs, prtclIndxs);
	}

	public boolean getInDistress(final long refSecs, final ParticleIndexes prtclIndxs) {
		return _particlesFile.getDistressRefSecs(prtclIndxs) <= refSecs;
	}

	public boolean getInDistressAndLanded(final long refSecs, final ParticleIndexes prtclIndxs) {
		final boolean inDistress = _particlesFile.getDistressRefSecs(prtclIndxs) <= refSecs;
		if (!inDistress) {
			return false;
		}
	...
	public int[] getSelectedObjectTypeIds() {
		final int nObjectTypes = _objectTypeToTotalWeightsByStage == null ? 0 : _objectTypeToTotalWeightsByStage.size();
		if (nObjectTypes == 0) {
			return null;
		}
		final Integer[] intArray = _objectTypeToTotalWeightsByStage.keySet().toArray(new Integer[nObjectTypes]);
	...
	public double objectTypeAndStageToWeight(final int objectType, final int stage) {
		final double[] weights = _objectTypeToTotalWeightsByStage.get(objectType);
	...
	/**
	 * Sim does not have a pos function. So to retrieve all of the particles, we
	 * need the following.
	 */
