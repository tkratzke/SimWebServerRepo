 ...
 */
public class NetCdfCurrentsUvGetter extends NetCdfUvGetter
		implements CurrentsUvGetter {
	final private SimCaseManager.SimCase _simCase;
	private boolean _haveLoggedInvalid;

	public NetCdfCurrentsUvGetter(final SimCaseManager.SimCase simCase,
			final Model model, final String filePath, final double dU,
		...
		super(simCase, model, _DriftsTag, filePath,
				new String[] { "U", "u", "water_u" },
				new String[] { "V", "v", "water_v" },
				new String[] { "Speed", "speed", "water_speed" },
				new String[] { "dir", "Dir", "direction", "Direction", "water_dir",
						"water_direction" },
				new String[] { "U_Unc", "u_Unc", "u_unc", "water_u_Unc" },
				new String[] { "V_Unc", "v_Unc", "v_unc", "water_v_Unc" },
				new String[] { "U_AltUnc", "u_AltUnc", "u_altunc",
						"water_u_AltUnc" },
				new String[] { "V_AltUnc", "v_AltUnc", "v_altunc",
						"water_v_AltUnc" },
				new String[] { "riverid" }, new String[] { "seqno" },
				new String[] { "flag" }, dU, dV, altDU, altDV, halfLifeSecs,
				preDistressHalfLifeSecs, true);
		_simCase = simCase;
		_haveLoggedInvalid = false;
	}

	public NetCdfCurrentsUvGetter(final SimCaseManager.SimCase simCase,
			final Model model, final String nominalFilePath,
			final NetcdfFile netCdfFile, final double defaultDU,
			final double defaultDV, final double defaultAltDU,
			final double defaultAltDV, final long halfLifeSecs,
			final long preDistressHalfLifeSecs) throws NetCdfUvGetterException {
		super(simCase, model, _DriftsTag, nominalFilePath, netCdfFile,
				new String[] { "U", "u", "water_u" },
				new String[] { "V", "v", "water_v" },
				new String[] { "Speed", "speed", "water_speed" },
				new String[] { "dir", "Dir", "direction", "Direction", "water_dir",
				...
				new String[] { "V_AltUnc", "v_AltUnc", "v_altunc",
						"water_v_AltUnc" },
				new String[] { "riverid" }, new String[] { "seqno" },
				new String[] { "flag" }, defaultDU, defaultDV, defaultAltDU,
				defaultAltDV, halfLifeSecs, preDistressHalfLifeSecs,
	...
	@Override
	public DataForOnePointAndTime getCurrentData(final long refSecs,
			final LatLng3 latLng, final String interpolationMode) {
		return getDataForOnePointAndTime(_simCase, refSecs, latLng,
				interpolationMode);
	}
	 ...
	 * routine from {@link com.skagit.sarops.environment.CurrentsUvGetter}.
	 */
	@Override
	...
	public boolean isEmpty(final SimCaseManager.SimCase simCase) {
		...
		if (empty && !_haveLoggedInvalid) {
			SimCaseManager.err(simCase, "Invalid Drifts UvGetter!");
