...
/**
 * Represents the data of a {@link Particle Particle}that is in distress.
 */
public class DistressStateVector extends StateVector {
	...
	public DistressStateVector(final Particle particle, final long simSecs,
			final LatLng3 latLng, final boolean updateParticleTail) {
		super(particle, simSecs, latLng, StateVectorType.DISTRESS,
				updateParticleTail);
		_zSeaNorth = _zSeaEast = _zWindNorth = _zWindEast = Float.NaN;
	}

	public DistressStateVector(final StateVector predecessor,
			final long simSecs, final boolean updateParticleTail) {
		super(predecessor, StateVectorType.DISTRESS, simSecs,
				updateParticleTail);
		_zSeaNorth = _zSeaEast = _zWindNorth = _zWindEast = Float.NaN;
	}
	...
	public StateVector timeUpdate(final Tracker tracker,
		...
		/**
		 * We need to get the CurrentsUvGetter so we can adjust, if necessary,
		...
		final boolean noAuxiliaryProcessing =
				currentSimSecs >= simSecs || isStuckOnLand() || isAnchored() ||
						!currentsUvGetter.hasAuxiliaryProcessing();
		if (noAuxiliaryProcessing) {
			return coreTimeUpdate(tracker, scenario, currentsUvGetter,
					windsUvGetter, simSecsS, iT, /* updateParticleTail= */true);
		}
		final DistressStateVector distressStateVector =
				currentsUvGetter.fillInStateVectorsIfAppropriate(simCase,
						windsUvGetter, scenario, simSecsS, this, simSecs);
		return distressStateVector;
	}

	final public DistressStateVector coreTimeUpdate(final Tracker tracker,
			final Scenario scenario, final CurrentsUvGetter currentsUvGetter,
			final WindsUvGetter windsUvGetter, final long[] simSecsS,
			final int iT, final boolean updateParticleTail) {
		final SimCaseManager.SimCase simCase = tracker.getSimCase();
		...
		if (currentSimSecs >= simSecs) {
			/**
			 * This happens when particles are initially at different times when
			 * we do the first time updates. We need some of them to catch up.
			 */
			return this;
		}
		final Particle particle = getParticle();
		...
		final LatLng3 latLng0 = getLatLng();
		--particle._remainingPenalty;
		if (isStuckOnLand() || particle._remainingPenalty > 0) {
			final DistressStateVector newStateVector =
					new DistressStateVector(this, simSecs, updateParticleTail);
		...
		if (currentsUvGetter != null && !currentsUvGetter.isEmpty(simCase)) {
			final boolean forCurrents = true;
			final String interpolationModeD =
					model.getInterpolationMode(forCurrents);
			final DataForOnePointAndTime currentsUv = currentsUvGetter
					.getCurrentData(refSecsForEnvLookUp, latLng0, interpolationModeD);
			/* Set the draw to zero if ... */
			final double zDrawNorth;
			final double zDrawEast;
			if (!particle.isEnvMean() &&
					currentsUvGetter.useRandomDuringUpdates()) {
			...
			if (!Double.isNaN(_zSeaNorth)) {
				/**
				 * rho is 1 for riverine downstream and 0 for riverine cross-stream.
				 * It depends on the half life otherwise.
				 */
				if (riverine) {
					newZEast = _zSeaEast;
					...
					final double rho;
					if (particle._fullPenalty > 0) {
						/** Just emerged from penalty box; rho is 0. */
						rho = 0d;
					} else {
						final double currentHalfLifeSecs =
								currentsUvGetter.getHalfLifeSecs(overallIndex);
						if (currentHalfLifeSecs > 0d) {
							final double currentDecayRatePerSec =
									_Ln2 / currentHalfLifeSecs;
			...
			z[0] = newZEast;
			z[1] = newZNorth;
			final float currentsDu =
					currentsUv.getValue(NetCdfUvGetter.DataComponent.DU);
			final float currentsDv =
					currentsUv.getValue(NetCdfUvGetter.DataComponent.DV);
			...
			if (!riverine) {
				...
				/**
				 * River case: We interpret the zSeaEast and zSeaNorth as
				 * down-stream and cross-stream. Moreover, uStandardDeviation is a
				 * ratio, and vStandardDeviation is a fraction of that ratio.
				 */
				if (model.getReverseDrift()) {
					u = -u;
					v = -v;
				}
				final double[] unitDownstream = new double[] { u, v };
				final double downstreamSpeed =
						NumericalRoutines.convertToUnitLength(unitDownstream);
				...
				if (!Double.isNaN(unitDownstream[0])) {
					final double[] unitCrossStream =
							new double[] { -unitDownstream[1], unitDownstream[0] };
					final double downstreamPerturbation = newZEast * downStreamSd;
					final double crossStreamPerturbation = newZNorth * crossStreamSd;
					du = downstreamPerturbation * unitDownstream[0] +
							crossStreamPerturbation * unitCrossStream[0];
					dv = downstreamPerturbation * unitDownstream[1] +
							crossStreamPerturbation * unitCrossStream[1];
				} else {
					du = dv = 0;
				}
		...
		if (windsUvGetter != null && !windsUvGetter.isEmpty(simCase)) {
			final String interpolationModeW =
					model.getInterpolationMode(/* forCurrents= */false);
			final DataForOnePointAndTime dnWindData =
					windsUvGetter.getDownWindData(refSecsForEnvLookUp, latLng0,
							interpolationModeW);
			/** Perturb the wind velocity. */
			final double zDrawEast;
			final double zDrawNorth;
			if (!particle.isEnvMean() && windsUvGetter.useRandomDuringUpdates()) {
				zDrawNorth = random.getTruncatedGaussian();
				zDrawEast = random.getTruncatedGaussian();
			} else {
				zDrawNorth = 0;
			...
			if (!Double.isNaN(_zWindNorth)) {
				...
				final double rho;
				if (particle._fullPenalty == 0 && windHalfLifeSecs > 0d) {
			...
			final float dnWindDu =
					dnWindData.getValue(NetCdfUvGetter.DataComponent.DU);
			final float dnWindDv =
					dnWindData.getValue(NetCdfUvGetter.DataComponent.DV);
			final float dnWindEastSpeed =
					(float) (downWindU + newZEast * dnWindDu);
			final float downWindNorthSpeed =
					(float) (downWindV + newZNorth * dnWindDv);
			final double[] eastAndNorthLeewaySpeeds =
					leewayCalculator.getEastAndNorthLeewaySpeeds(dnWindEastSpeed,
							downWindNorthSpeed, simSecs, particle);
		...
		/**
		 * Deal with land. We now have a target latLng if land does not get in
		 * the way. Recall that we are starting not stuck on land.
		 */
		final long mySimSecs = getSimSecs();
		final TangentCylinder myTangentCylinder =
				particle.getTangentCylinderFromSimSecs(latLng0, mySimSecs);
		final TangentCylinder.FlatLatLng initialFlatLatLng =
				myTangentCylinder.convertToMyFlatLatLng(latLng0);
		final LatLng3 latLngAfterWind = LatLng3.makeBasicLatLng3(
				initialFlatLatLng.addOffsets(eastMoveR, northMoveR));
		final boolean isSticky = model.getIsSticky(overallIndex);
		final ShorelineFinder shorelineFinder = model.getShorelineFinder();
		double hdg = MathX.initialHdgX(latLng0, latLngAfterWind);
		final double completeHaversine =
				MathX.haversineX(latLng0, latLngAfterWind);
		final double completeMeters = completeHaversine / _MToR;
		/** Sticky is relatively easy; just stick to the landing or float on. */
		if (isSticky) {
			final StopAndBlock stopAndBlock =
					new StopAndBlock(logger, shorelineFinder, latLng0, hdg,
							completeMeters, _MInc, /* debug= */false);
			if (stopAndBlock.ranIntoSomething()) {
				newStateVector.setLatLng(stopAndBlock._stopLatLng);
				newStateVector.setIsStuckOnLand();
				particle._fullPenalty =
						particle._remainingPenalty = Integer.MAX_VALUE;
			} else {
		...
		for (int k0 = 0; k0 < _NTimesToIterateBeforeGivingUp; ++k0) {
			...
			final StopAndBlock stopAndBlock0 =
					new StopAndBlock(logger, shorelineFinder, latLng, hdg, remainingM,
							_MInc, /* debug= */false);
			if (stopAndBlock0.ranIntoSomething()) {
				/**
				 * Got blocked. Update remainingM. We always give credit for at
				 * least _MInc.
				 */
			...
			final double idealHdg = NumericalRoutines
					.generalMod(MathX.initialHdgX(latLng, oldLatLng) + 180d, 360d);
			if (k0 < _NTimesToUseIdealHdg) {
				final GreatCircleArc blockingGca0 =
						stopAndBlock0._blockingSfr.getBlockingGca();
				final TangentCylinder tc =
						TangentCylinder.getTangentCylinder(latLng);
				final TangentCylinder.FlatLatLng flatBlockingLatLng0 =
						tc.convertToMyFlatLatLng(blockingGca0.getLatLng0());
				final TangentCylinder.FlatLatLng flatBlockingLatLng1 =
						tc.convertToMyFlatLatLng(blockingGca0.getLatLng1());
				...
				final TangentCylinder.FlatLatLng shiftedFlatLatLngA =
						tc.new FlatLatLng(shiftedA[0], shiftedA[1]);
				final TangentCylinder.FlatLatLng shiftedFlatLatLngB =
						tc.new FlatLatLng(shiftedB[0], shiftedB[1]);
				...
				/**
				 * We have given up on using an ideal hdg. Look for a hdg that
				 * maximizes the distance we can move from latLng, or is at least
				 * big enough to make significant progress towards the goal. Our
				 * definition of "significant progress" grows more and more
				 * restrictive, since the distance we need to move gets smaller. In
				 * what follows, p is 1/2, 3/4, 7/8, ...
				 */
				final double p = 1d - Math.pow(0.5, k0 + 1 - _NTimesToUseIdealHdg);
				final double thresholdM = p * remainingM;
				/**
				 * If we find a direction that admits a move at least remainingR,
				 * take it. Otherwise, simply take the direction that will admit the
				 * largest move.
				 */
				double bestM = 0d;
				double bestHdg = idealHdg;
				for (double hdgShift = 5d; Math.abs(hdgShift) <= 45d;
						hdgShift = -hdgShift + (hdgShift < 0d ? 5d : -5d)) {
					final double thisHdg =
							NumericalRoutines.generalMod(hdg + hdgShift, 360d);
					final StopAndBlock stopAndBlock1 =
							new StopAndBlock(logger, shorelineFinder, latLng, thisHdg,
					...
					final double thisM =
							MathX.haversineX(latLng, stopAndBlock1._stopLatLng) / _MToR;
					if (thisM > thresholdM) {
						bestHdg = thisHdg;
						break;
					}
		...
		if (clearedLand || riverine) {
			particle._fullPenalty = particle._remainingPenalty = 0;
		} else {
			particle._fullPenalty =
					(int) Math.round(1.5 * (particle._fullPenalty + 1d));
			particle._remainingPenalty = particle._fullPenalty;
		}
		return newStateVector;
	}
	...
	public String getDescription() {
		if (isStuckOnLand()) {
			return "Distress Landed particle of type " +
					getSearchObjectType().getId();
		}
		if (isAnchored()) {
			return "Distress Anchored particle of type " +
					getSearchObjectType().getId();
		}
	...
	/**
	 * Shifts the segment (in the plane) from A->B to ShiftedA->ShiftedB,
	 * where the latter is parallel to the former, but goes through the
	 * origin, and the projection of the origin onto the former is mapped to
	 * the origin.
	 */
	private static double[][] getShiftedEndpoints(final double[] A,
			final double[] B) {
		final double Ax = A[0];
		final double Ay = A[1];
		final double Bx = B[0];
		final double By = B[1];
		final double[] shiftedXy = new double[] { -Ax, -Ay };
		/**
		 * The perp is (-deltaY, deltaX). It's the important vector and we form
		 * it here without forming the regular unit vector.
		 */
		final double[] u = new double[] { Bx - Ax, By - Ay };
		final double[] v = new double[] { -u[1], u[0] };
		/**
		 * To avoid rounding, we do not normalize u or v; this means we have to
		 * divide by v's length when we form the dot product, and then by v's
		 * length again when we apply this dot product to v. Hence, the strange
		 * looking scalar "dotProduct(...)/lengthSq."
		 */
		final double lengthSq = v[0] * v[0] + v[1] * v[1];
		final double yPrime =
				NumericalRoutines.dotProduct(v, shiftedXy) / lengthSq;
		...
		return new double[][] { new double[] { shiftedAx, shiftedAy },
				new double[] { shiftedBx, shiftedBy } };
	}
}
